...
v1.5.2 说明：旧 `output` 会在缺少新字段时迁移为 `children_output`。电视剧、电影和儿童是三个独立目标；TMDB/豆瓣与 DeepSeek 分类低于 0.90、相互冲突或不可用时只预览、不移动。配置页增加小视口滚动与路径换行，高级设置默认折叠；运行日志可按 `CourseOrganizer[event=...]` 检索完整处理阶段。

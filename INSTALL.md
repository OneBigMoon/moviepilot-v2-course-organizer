...
v1.5.4 说明：旧 `output` 会在缺少新字段时迁移为 `children_output`。电视剧、电影和儿童是三个独立目标；TMDB/豆瓣与 DeepSeek 分类低于 0.90、相互冲突或不可用时只预览、不移动。配置页增加小视口滚动与路径换行，高级设置默认折叠；运行日志可按 `CourseOrganizer[event=...]` 检索完整处理阶段，其中 `scan_started` 默认以 INFO 级别记录，便于与 `scan_completed` 组成可见证据链；一次性运行请求在排队期间保持为 true，在回调取得执行权或显式停止时复位为 false，配置重载不会取消已排队回调。
